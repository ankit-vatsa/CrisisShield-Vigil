function submitData() {
  var newzipName = document.getElementById("zipcodeInput").value;
  var newcountryName = document.getElementById("countryInput").value;

  var data = {
      "httpMethod": "POST",
      "body": {
          "zipCode": String(newzipName),
          "countryCode": String(newcountryName)
      }
  };
  // var data = {
  //     "httpMethod": "POST",
  //     "body": {
  //         "zipCode": newzipName.value,
  //         "countryCode": newcountryName.value
  //     }
  // };
...
function GetInfo() {
    ...
    var newzipName = document.getElementById("zipcodeInput");
    var countryIDName = document.getElementById("countryIDName");
    var newcountryName = document.getElementById("countryInput");
    zipcodeName.innerHTML = "--"+newzipName.value+"--";
    countryIDName.innerHTML = "--"+newcountryName.value+"--";
    fetch('https://api.openweathermap.org/data/2.5/forecast?zip='+newzipName.value+','+newcountryName.value+'&appid=745c166d888019b4ce200a58469f67da')
    .then(response => response.json())
    .then(data => {
